(function () {
  ...
  const KB = {
    ...
    // ── GREETING ────────────────────────────────────────────────────────────
    greeting: {
      keywords: ['bonjour', 'bonsoir', 'salut', 'allo', 'hello', 'hi', 'hey', 'good morning', 'good evening', 'coucou', 'hola'],
      response: () => t(
    ...
    // ── PRICING / VALUATION ──────────────────────────────────────────────────
    pricing: {
      keywords: ['prix', 'price', 'combien', 'how much', 'valeur', 'value', 'vaut', 'worth', 'estimate', 'estimation', 'cote', 'côte', 'coter', 'market', 'marché', 'secondary', 'cotes'],
      response: () => t(
    ...
    // ── SELL / BUYBACK ───────────────────────────────────────────────────────
    sell: {
      keywords: ['vendre', 'sell', 'céder', 'revendre', 'racheter', 'rachat', 'buyback', 'buy my', 'je veux vendre', 'how to sell', 'comment vendre', 'offre de rachat', 'get an offer'],
      response: () => t(
    ...
    // ── ROLEX ────────────────────────────────────────────────────────────────
    rolex: {
      keywords: ['rolex', 'submariner', 'daytona', 'datejust', 'gmt master', 'gmt-master', 'explorer', 'sky-dweller', 'milgauss', 'yacht-master', 'oyster perpetual'],
      response: () => t(
    ...
    // ── AUDEMARS PIGUET ──────────────────────────────────────────────────────
    ap: {
      keywords: ['audemars', 'piguet', 'royal oak', '15500', '15202', '15400', '26331', 'offshore', 'code 11.59'],
      response: () => t(
    ...
    // ── PATEK PHILIPPE ───────────────────────────────────────────────────────
    patek: {
      keywords: ['patek', 'philippe', 'nautilus', '5711', '5726', '5167', 'aquanaut', 'calatrava', 'grand complication', '5980'],
      response: () => t(
    ...
    // ── RICHARD MILLE ────────────────────────────────────────────────────────
    rm: {
      keywords: ['richard mille', 'richard', 'rm 011', 'rm 035', 'rm 055', 'rm 27', 'rm011', 'rm035'],
      response: () => t(
        `**Richard Mille** — l'avant-garde de l'horlogerie haute technologie.\n\n🏎️ **Modèles populaires :**\n• **RM 011** — Felipe Massa, flyback, ~€170k–€340k\n• **RM 035** — Ultraléger Rafael Nadal, ~€140k–€270k\n• **RM 055** — Bubba Watson, sans chiffres\n\n⚡ Matériaux aérospatiaux (NTPT, titane, carbone) associés à une mécanique de haute précision. Éditions très limitées — liquidité et valeur stables sur le marché secondaire.\n\n💡 Vous avez une Richard Mille ? Dites-moi le modèle pour une estimation !`,
        `**Richard Mille** — the avant-garde of high-tech watchmaking.\n\n🏎️ **Popular models:**\n• **RM 011** — Felipe Massa flyback, ~€170k–€340k\n• **RM 035** — Rafael Nadal ultralight, ~€140k–€270k\n• **RM 055** — Bubba Watson, no numerals\n\n⚡ Aerospace materials (NTPT, titanium, carbon) combined with precision mechanics. Very limited editions — stable liquidity and value on the secondary market.\n\n💡 Got a Richard Mille? Tell me the model for an estimate!`
      )
    },

    // ── CARTIER ──────────────────────────────────────────────────────────────
    cartier: {
      keywords: ['cartier', 'santos', 'tank', 'pasha', 'ballon bleu', 'panthère', 'drive de cartier'],
      response: () => t(
    ...
    // ── CONDITION / PAPERS ───────────────────────────────────────────────────
    condition: {
      keywords: ['condition', 'état', 'papers', 'papiers', 'box', 'boîte', 'sans papiers', 'without papers', 'avec papiers', 'with papers', 'unworn', 'neuve', 'used', 'occasion', 'mint', 'scratches', 'rayures'],
      response: () => t(
    ...
    // ── CONTACT / APPOINTMENT ────────────────────────────────────────────────
    contact: {
      keywords: ['contact', 'appointment', 'rendez-vous', 'rdv', 'meeting', 'visit', 'phone', 'téléphone', 'email', 'joindre', 'whatsapp', 'call', 'book', 'réserver'],
      response: () => t(
    ...
    // ── ABOUT NOS MONTRES ────────────────────────────────────────────────────
    about: {
      keywords: ['nos montres', 'qui êtes', 'who are you', 'qui sommes', 'about you', 'votre boutique', 'your shop', 'à propos', 'about', 'company', 'société', 'bruxelles', 'brussels', 'belgique', 'belgium', 'qui vous'],
      response: () => t(
  ...
  // ─── Worker price lookup ────────────────────────────────────────────────────
  // Returns Worker data if the user's message mentions a watch brand + is a price query
  function isPriceIntent(n) {
    return ['prix', 'price', 'combien', 'how much', 'valeur', 'value', 'vaut', 'worth',
            'estimate', 'estimation', 'cote', 'côte', 'coter'].some(kw => n.includes(kw));
  }

  function hasBrand(n) {
    return ['rolex', 'audemars', ' ap ', 'patek', 'richard mille', 'cartier', 'rm '].some(b => n.includes(b));
  }
  ...
  function matchKB(n) {
    for (const key in KB) {
      if (KB[key].keywords.some(kw => n.includes(norm(kw)))) {
  ...
  async function getResponse(text) {
    ...
    // 1. If it looks like a price query with a brand → try Worker first
    if ((isPriceIntent(n) || hasBrand(n)) && hasBrand(n)) {
  ...
  const qEn = [
    ...
    { q: 'Rolex prices models',             label: '⌚ Rolex' },
    { q: 'Audemars Piguet Royal Oak',       label: '🏆 AP' },
    { q: 'Patek Philippe Nautilus price',   label: '💎 Patek' },
    ...
    <div id="nm-chat-window" role="dialog" aria-label="Nos Montres Chat">
      <div id="nm-chat-header">
        ...
        <div id="nm-chat-header-info">
          ...
          <div id="nm-chat-header-status" id="nm-status">Expert · Estimation instantanée</div>
        </div>
